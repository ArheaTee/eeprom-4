    ...
    def run(self,):                            
        ...
        while True:           
            ...
            if employeeNumber == "golden":
                self.config_parser.set('OpenTestCloudReporter', 'SendDebugRecord', 'True')
                authorized = True
            ...
            with SyncObject(self,300.0,True) as shouldRun:        #sync all nests and run test in only one thread
                if shouldRun:                   
                    ...
                    if shouldUpdate:
                        ...
                        QueryUpdateHaleFirmware.MD5sums = UpdateDUT.queryHaleMD5sums( self.global_driver.DUT )                
            if QueryUpdateHaleFirmware.MD5sums is not None and self.station_id - 1 < len(QueryUpdateHaleFirmware.MD5sums):
