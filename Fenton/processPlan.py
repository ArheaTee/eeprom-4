...
LOGGER_NAME = 'Palomar'
STATION_NAME = 'Fenton-' + PROCESS_STEP
LOG_FILE_DIR = 'C:/'+STATION_NAME+'-LOG'
VERSION = '0.71'

HARDWARE_REVISION = "0a"
SN_PATTERN = re.compile(r"^PFF[A-Z]{3}\d{9}$")
    ...
    def run(self, ):
        ...
        try:
            ...
            try:
                ...
                goldenmodules = ["chabot","kepler", "griffith1", "griffith2", 
                ...
                uuts = ["fenton1", "fenton2", "fenton3", "fenton4"]
...
class Read_FRU_SN_PN(ProcessStep):
     ...
     def run(self,):
        try:
             ...
             dev, mux = EEPROM_FRU.DevMuxByName["Fenton%d" % self.station_id]
...
class InitializeOpenTestCloudReportVariables(ProcessStep):
   ...
   def run(self):
    ...
    #    self.set_reporter_variable('assembly_gpn', self.variables["assembly_part_number"])
       ...
       self.set_reporter_variable('product_name', 'Fenton')
       self.set_pass()
       return ProcessStatus.PASSED
...
class FanSpeedTest(ProcessStep):
    "Fan Speed Test"

    def __init__(self, processPlan, name="New Process Step", description="", timeout=5.0):
        super(FanSpeedTest, self).__init__(processPlan,name=name,description=description)
        self.timeout = timeout
    ...
    def run(self, ):
        ...
        try:
            ...
            fanID = 'fenton%d_2c_1' % self.station_id

            diagCmd = "fan set %d 50" % (self.station_id-1)
            self.global_driver.DUT.CommandQuery(diagCmd,self.timeout)
            time.sleep(settlingTime)
            diagCmd = "fan get %d" % (self.station_id-1)
            response = self.global_driver.DUT.CommandQuery(diagCmd,self.timeout)

            speed = int(response[response.find(fanID):].split()[2])

            if not addParametricResult(self, "fanSpeed_50%", speed, "RPM", medium_ul, medium_ll ):
                result = ProcessStatus.FAILED

            diagCmd = "fan set %d 100" % (self.station_id-1)
            self.global_driver.DUT.CommandQuery(diagCmd,self.timeout)
            time.sleep(settlingTime)
            diagCmd = "fan get %d" % (self.station_id-1)
            response = self.global_driver.DUT.CommandQuery(diagCmd,self.timeout)
            ...
            if not addParametricResult(self, "fanSpeed_100%", speed, "RPM", high_ul, high_ll ):
                result = ProcessStatus.FAILED

            if result == ProcessStatus.PASSED:
                self.set_pass()
...
class DiagI2CFenton(ProcessStep):
    "Fenton I2C Test"
    response = None

    def __init__(self, processPlan, name="New Process Step", description="", timeout=5.0):
        super(DiagI2CFenton, self).__init__(processPlan,name=name,description=description)
        self.timeout = timeout

    def run(self, ):
        self.global_driver = self.get_global_variable('Driver_System_Station.SharedDriver.Driver')

        DiagI2CFenton.response = None

        try:
            with SyncObject(self,self.timeout,True) as shouldRun:        #sync all nests and run test in only one thread
                if shouldRun:
                    DiagI2CFenton.response = self.global_driver.DUT.CommandQuery("diag_i2c fenton",self.timeout)

            response = DiagI2CFenton.response

            if "PASS" in response.upper() and not "FAIL" in response.upper():
                self.set_pass()
                return ProcessStatus.PASSED
            else:
                searchString1 = 'on bus(5) mux(%d)' % (self.station_id*2 + 6)  #station1 = 8,9  / station2 = 10,11 / station3 = 12,13 / station4 = 14,15
                searchString2 = 'on bus(5) mux(%d)' % (self.station_id*2 + 7)
                if searchString1 in response or searchString2 in response:
                    self.set_fail("diag_i2c fenton", "Unexpected response from diagnostic command 'diag_i2c fenton' (see Debug Output for detail)")
                    self.log( response )
                    if self.global_driver.DUT.saveFailResponses:
                        self.global_driver.DUT.LogFailResponse("diag_i2c fenton",response)
                    return ProcessStatus.FAILED
                else:
                    searchString = ' bus(5)  mux(%d)' % (self.station_id*2 + 6)
                    if searchString in response:
                        self.set_pass()
                        return ProcessStatus.PASSED
                    else:
                        self.set_fail("diag_i2c fenton","Unexpected response from diagnostic command 'diag_i2c core' (see Debug Output for detail)")
                        self.log( response )
        except Exception, ex:
            self.set_fail("diag_i2c fenton", traceback.format_exc() )
            return ProcessStatus.FAILED

class DiagIntrFenton(ProcessStep):
    "Fenton Intr Test"
    response = None

    def __init__(self, processPlan, name="New Process Step", description="", timeout=30.0):
        super(DiagIntrFenton, self).__init__(processPlan,name=name,description=description)
        self.timeout = timeout

    def run(self, ):
        self.global_driver = self.get_global_variable('Driver_System_Station.SharedDriver.Driver')

        DiagIntrFenton.response = None

        try:
            with SyncObject(self,self.timeout,True) as shouldRun:        #sync all nests and run test in only one thread
                if shouldRun:
                    DiagIntrFenton.response = self.global_driver.DUT.CommandQuery("diag_intr ignore present",self.timeout)

            response = DiagIntrFenton.response

            if "PASS" in response.upper() and not "FAIL" in response.upper():
                self.set_pass()
                return ProcessStatus.PASSED
            else:
                searchString1 = 'testing interrupt 6:%d' % (self.station_id+25)  
                searchString2 = 'Missing presence for interrupt 6:%d' % (self.station_id+25)  
...
class FanTest(ProcessStep):
    ...
    def __init__(self, processPlan, name="New Process Step", description="", timeout=240.0):
                ...
                return ProcessStatus.PASSED
                 ...
                 with SyncObject(self,self.timeout,True):
                    ...
                    if "PASS" in response.upper() and not "FAIL" in response.upper():
                        ...
                        self.set_fail(self.diagCmd, "Diagnostic command 'fan test %d' response indicates failure (see Debug Output for detail)" % fanIndex)
                        self.log( response )                
                        if self.global_driver.DUT.saveFailResponses:                    
                            self.global_driver.DUT.LogFailResponse(self.diagCmd,response)
                        return ProcessStatus.FAILED
                    else:                            
                        self.set_fail("fan test %d" % fanIndex, "Unexpected response from diagnostic command 'fan test %d' (see Debug Output for detail)" % fanIndex )
                        self.log( response )
                        if self.global_driver.DUT.saveFailResponses:
                            self.global_driver.DUT.LogFailResponse("fan test %d" % fanIndex,response)
                        return ProcessStatus.FAILED       
        except Exception, ex:              
...
class DiagCommandStep(ProcessStep):
    ...
    def __init__(self, processPlan, name="New Process Step", description="", diagCmd="",timeout=0.5,ignoreErrors=False):
...
class TemperatureTest(ProcessStep):
    ...
    def run(self, ):
        ...
        diagCmd = "temp test %d" % (channel+10)
...
class TemperatureDump(ProcessStep):
    "Fenton temperature sensor dump"
    ...
    def run(self, ):
        ...
        diagCmd = "temp dump %d" % (channel+10)

        try:
            
            response = self.global_driver.DUT.CommandQuery(diagCmd,2)
            fru_name = "fenton%d" % (channel+1)
...
class CleanupProcessStep(ProcessStep):
    ...
    def GenerateForCM_JSON(self):
        ...
        boardInfo["subla_gpn_name"] = "Fenton"
    ...
    def run(self, ):
        ...
        try:
            ...
            self.station_driver = self.get_station_variable('Driver_System_Station.StationDriver.Driver')
            cycleCount = self.station_driver.IncrementCycleCount( "CycleCount" % (self.getChassisId()+1), "Fenton%d" % self.station_id )
...
class MainProcessPlan(ProcessPlan):
    ...
    def CreateProcessPlan(self, ):
        ...
        #myPlan.append( OperatorInstruction(self,"PromptInsertFentons", "Display Insert Fentons Prompt", dialogTitle="Insert Fentons", prompt="Click OK when all Fentons are inserted")) 
        ...
        myPlan.append(DiagI2CFenton(self,'Diag I2C Fenton', 'Execute I2C Fenton Test'))
        
        myPlan.append(DiagIntrFenton(self,'Diag Interrupt', 'Interrupt diagnostic test'))

        myPlan.append(DiagFanTray(self,'Diag Fan Tray', 'Execute Fan Tray Test', timeout=120))
        ...
        myPlan.append(FanTest(self,'Diag Fan Test', 'Execute Fan Test'))

        #myPlan.append(FanSpeedTest(self, 'Fan Speed Test', 'Verify Medium and High Fan Speed'))
