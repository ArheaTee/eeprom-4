...
from server.ProcessObjects import DataRecord, DataRecordType
...
from server.Station import ProcessStationStatus
    ...
    def run(self,):                            
        ...
        while True:           
            ...
            try:
                ...
                authorized = False
            if employeeNumber == "golden":

                self.config_parser.set(

                    'OpenTestCloudReporter', 'SendDebugRecord', 'True')
                    ...
                    while True:
                        ...
                        if m and m.group() == part_num and part_num[:8] in EEPROM_FRU.NameByPartNumber:
                            ...
                            if not self.expectedBoardType is None and not self.expectedBoardType == boardType:
                                ...
                                self.variables['product_name'] = boardType
                                if boardType in "Fenton":
                                    self.process_plan.process_plan_name = "Fenton EEPROM Plan"
                                elif boardType in "Hale":
                                    self.process_plan.process_plan_name = "Hale EEPROM Plan"
...
class InitializeOpenTestCloudReportVariables(ProcessStep):
   ...
   def run(self):
       ...
       self.set_reporter_variable('product_name', self.variables['board_type'])
...
class PowerUp(ProcessStep):
    ...
    def run(self, ):
        ...
        try:
            self.global_driver = self.get_global_variable('Driver_System_Station.SharedDriver.Driver')
            for i in range(5):
                if self.global_driver.DUT.mode == "Unknown":
                    self.global_driver.DUT.GetMode(True)
            if self.global_driver.DUT.mode == "Unknown":
                self.global_driver.DUT.mode = "Off"
...
class QueryDiagFirmware(ProcessStep):
    ...
    def run(self, ):
        ...
        try:
            ...
            # CTH added for opentest cloud process run overview.
            self.set_reporter_variable('assembly_software_version', revision)

            self.add_data_record(DataRecord(DataRecordType.SOFTWARE, '', 'Diag Firmware', revision))
            self.add_measurement(Measurement("Diag Firmware Revision", revision, "", is_parametric=False, is_numeric=False))
        except Exception, ex:
            self.set_fail("QueryDiagFirmware", repr(traceback.format_exc()) )
            return ProcessStatus.FAILED
            
        self.set_pass()
        return ProcessStatus.PASSED

class QueryNellieFPGAFirmware(ProcessStep):
    ''''''
    @syncRunSingle
    def run(self, ):
        ...
        try:
            ...
            self.add_data_record(DataRecord(DataRecordType.SOFTWARE, '', 'Nellie FPGA Firmware', revision))
            self.add_measurement(Measurement("Nellie FPGA Firmware Revision", revision, "", is_parametric=False, is_numeric=False))
        except Exception, ex:
            self.set_fail("QueryNellieFPGAFirmware", repr(traceback.format_exc()) )
            return ProcessStatus.FAILED
...
class QueryLeahFPGAFirmware(ProcessStep):
    ''''''
    @syncRunSingle
    def run(self, ):
        ...
        try:
            ...
            self.add_data_record(DataRecord(DataRecordType.SOFTWARE, '', 'Leah FPGA Firmware', revision))
            self.add_measurement(Measurement("Leah FPGA Firmware Revision", revision, "", is_parametric=False, is_numeric=False))
        except Exception, ex:
            self.set_fail("QueryLeahFPGAFirmware", repr(traceback.format_exc()) )
            return ProcessStatus.FAILED
...
class QueryUBoot(ProcessStep):
    ...
    def run(self, ):
        ...
        try:
            ...
            self.add_data_record(DataRecord(DataRecordType.SOFTWARE, '', 'UBoot', revision))
            self.add_measurement(Measurement("UBoot Revision", revision, "", is_parametric=False, is_numeric=False))
        except Exception, ex:
            self.set_fail("QueryUBoot", repr(traceback.format_exc()) )
            return ProcessStatus.FAILED

        self.set_pass()
        return ProcessStatus.PASSED

class QueryNetlo(ProcessStep):
    ''''''
    @syncRunSingle
    def run(self, ):
        ...
        self.global_driver = self.get_global_variable('Driver_System_Station.SharedDriver.Driver')
        try:
            revision = UpdateDUT.queryNetloRevision( self.global_driver.DUT )

            self.add_data_record(DataRecord(DataRecordType.SOFTWARE, '', 'Netlo', revision))
            self.add_measurement(Measurement("Netlo Revision", revision, "", is_parametric=False, is_numeric=False))
        except Exception, ex:
            self.set_fail("QueryNetlo", repr(traceback.format_exc()) )
            return ProcessStatus.FAILED
...
class CleanupProcessStep(ProcessStep):
    ...
    _configFileLock = threading.RLock()
    ...
    def check_for_consecutive_failures(self, slotName):                       
            ...
            try:
                ...
                if not "DISABLED" in station_name.upper():
                    ...
                    try:
                        ...
                        if self._process_plan.result.StatusResult != ProcessStatus.PASSED:
                            try:
                                fail_count = int(config_parser['ConsecutiveFailCount'][slotName])
                            except:
                                fail_count = 0

                            fail_count += 1                        

                            config_parser['ConsecutiveFailCount'][slotName] = str(fail_count)
                            ...
                            if fail_count >= consecutive_fail_limit:
                                ...
                                self._process_plan._global_execution_context['process_stations'][self.station_id]._station_name =  DISABLED_STATION_PREFIX + station_name
                        else:
                            config_parser['ConsecutiveFailCount'][slotName] = "0"
                    finally:
                        config_parser.write()
            except Exception, ex:
                try:
    ...
    def run(self, ):
        ...
        try:
            ...
            try:
                self.check_for_consecutive_failures(slotName)
            except:
                pass
...
class MainProcessPlan(ProcessPlan):
    ...
    def __init__(self, station_execution_context, global_execution_context):
        super(MainProcessPlan, self).__init__(
            station_execution_context, global_execution_context)
        self._process_plan_name = 'EEPROM'
    ...
    def process_plan_name(self):
        ...
        """
        return self._process_plan_name

    @process_plan_name.setter
    def process_plan_name(self, value):
        """Sets the process plan name.
        """
        self._process_plan_name = value
    ...
    def execute_cleanup(self, ):
        ...
        subprocess.call([r'C:\Palomar\System\SendingSpectrumpro.bat'])
    ...
    def CreateProcessPlan(self, ):
        ...
        myPlan.append(QueryDiagFirmware(self,"Query Diag Firmware", "Queries diag firmware version"))
        myPlan.append(QueryUBoot(self,"Query Update UBoot", "Queries UBoot version"))
        myPlan.append(QueryNetlo(self,"Query Update Netlo", "Queries Netlo version"))        
        myPlan.append(QueryLeahFPGAFirmware(self,"Query Update Leah FPGA", "Queries Leah FPGA version"))
        myPlan.append(QueryNellieFPGAFirmware(self,"Query Update Nellie FPGA", "Queries Nellie FPGA version"))
